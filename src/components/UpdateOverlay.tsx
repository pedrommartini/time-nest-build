...
export const UpdateOverlay: React.FC<UpdateOverlayProps> = ({ onUpdate, onDismiss, version = '2.0.0' }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[200] flex items-center justify-center p-6 transition-all duration-300 ${isVisible ? 'bg-black/40 backdrop-blur-sm opacity-100' : 'bg-transparent backdrop-blur-none opacity-0 pointer-events-none'}`}>
      <div className={`w-full max-w-sm modal-standard overflow-hidden transition-all duration-400 transform ${isVisible ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-8 opacity-0'}`}>
        ...
        <div className="p-8 flex flex-col items-center text-center">
          ...
          <button 
            ...
            className="w-full py-4 btn-primary text-sm flex items-center justify-center gap-2"
          >
          ...
          <button 
            onClick={handleDismiss}
            className="mt-4 px-4 py-2 btn-tertiary text-xs text-text-secondary hover:text-text-primary"
          >
            Lembrar mais tarde
          </button>
        </div>
