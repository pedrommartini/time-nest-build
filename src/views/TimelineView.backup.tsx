...
import { Play, Search, Mic, CalendarDays, Menu, ChevronUp, Clock, Moon, Sparkles } from 'lucide-react';
...
export const TimelineView: React.FC = () => {
  ...
  return (
    <div className="relative h-full flex flex-col bg-app-bg overflow-hidden animate-fade-in">
      ...
      <div className="z-40 bg-app-bg/80 backdrop-blur-md pt-6 pb-4 px-5 border-b border-border-color/50 flex gap-3 items-center shrink-0">
        ...
        <div className="flex-1 relative flex items-center">
          ...
          <input
            ...
            className="input-standard pl-11 pr-12 text-sm cursor-text !rounded-full"
          />
          <button type="button" onClick={() => { openSmartInput(true); }} className="absolute right-1 w-10 h-10 btn-primary !rounded-full flex items-center justify-center !p-0">
      ...
      >
        <div className="relative w-full" style={{ height: '1403520px' }}>
          ...
          {Array.from({ length: visibleRange.end - visibleRange.start + 1 }).map((_, i) => {
            ...
            return (
              <div key={absoluteHour} className="absolute w-full flex items-center" style={{ top: absoluteHour * 80 - 8 }}>
                <span className={`text-[12px] w-[64px] text-right pr-3 ${
          ...
          {currentTimeStr && (
            <div 
              ...
              style={{ top: timeToOffsetPx(currentTimeStr) - 10 }}
            >
              <div className="relative flex items-center">
                <div className="w-[66px] flex flex-col items-center justify-center bg-app-bg py-1 z-10 relative">
                  <span className="text-[9px] font-bold uppercase tracking-widest text-red-500 mb-0.5">AGORA</span>
                  <span className="text-sm font-extrabold text-red-500">{currentTimeStr}</span>
                </div>
                <div className="flex-1 h-[2px] bg-red-500 relative mr-4 z-0"></div>
                <div className="absolute left-[70px] top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)] z-20"></div>
              </div>
            </div>
          )}
      ...
      {!isActive && (
        <div 
          className={`absolute bottom-0 left-0 right-0 bg-[#FDFDFB] dark:bg-card-bg z-40 rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.06)] border-t border-border-color flex flex-col transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${
            isDrawerExpanded ? 'h-[50vh]' : 'h-[75px]'
          }`}
          onClick={() => { if (!isDrawerExpanded) setIsDrawerExpanded(true); }}
        >
          <>
            <div className="w-10 h-1.5 bg-border-color/60 rounded-full mx-auto mt-3 mb-1 shrink-0 cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsDrawerExpanded(!isDrawerExpanded); }}></div>
            ...
            <div className="px-6 pb-2 pt-1 shrink-0 flex items-center justify-between cursor-pointer" onClick={(e) => { e.stopPropagation(); setIsDrawerExpanded(!isDrawerExpanded); }}>
              <div className="flex items-center gap-2">
                <h3 className="text-[19px] font-semibold text-text-primary">Tarefas disponíveis</h3>
                {!isActive && (
                  <span className="w-6 h-6 rounded-full bg-brand-500 text-white text-[12px] font-bold flex items-center justify-center shadow-sm">
                    {tasks.filter(t => t.status === 'pending' && t.estimatedDuration <= availableMinutes).length}
                  </span>
                )}
              </div>
              <ChevronUp className={`w-5 h-5 text-text-secondary transition-transform ${isDrawerExpanded ? 'rotate-180' : 'rotate-180'}`} />
            </div>
            
            <div className={`px-6 flex-1 overflow-y-auto custom-scrollbar flex flex-col pb-[110px] transition-opacity duration-200 ${isDrawerExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <p className="text-[14px] text-text-secondary mb-5 shrink-0 font-medium">
                Escolha o que você quer fazer no seu tempo livre
              </p>
              
              <div className="flex flex-col gap-0 border border-border-color/40 rounded-[20px] overflow-hidden bg-white dark:bg-card-bg">
                {tasks
                  .filter(t => t.status === 'pending' && t.estimatedDuration <= availableMinutes)
                  .map((task, index) => (
                  <div key={task.id} className={`flex items-center justify-between p-4 ${index > 0 ? 'border-t border-border-color/30' : ''}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[16px] font-medium text-text-primary mb-1">{task.title}</span>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${
                            task.size === 'Média' ? 'bg-brand-50 text-brand-600 dark:bg-brand-950/30 dark:text-brand-400' :
                    ...
                    <button 
                      ...
                      className="w-10 h-10 bg-white border border-border-color/60 rounded-full flex items-center justify-center shrink-0 active:scale-95 transition-transform"
                    >
              ...
              <button 
                ...
                className="mt-5 w-full py-2 text-brand-600 dark:text-brand-400 font-semibold text-[15px] hover:opacity-80 transition-opacity flex items-center justify-center gap-1.5 shrink-0"
              >
