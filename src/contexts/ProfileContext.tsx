...
export type EnergyLevel = 'Baixa' | 'Média' | 'Alta';
...
export const ProfileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [achievements, _setAchievements] = useState<Achievement[]>(() => {
    try {
      ...
      if (savedAchiev) return JSON.parse(savedAchiev);
    } catch(e) {}
  ...
  const [security, setSecurity] = useState<SecuritySettings>(() => {
    try {
      ...
      if (savedSec) return JSON.parse(savedSec);
    } catch(e) {}
    return {
  ...
  const [isLocked, setIsLocked] = useState<boolean>(() => {
    try {
      ...
      if (savedSec) {
        ...
        return parsed.passcodeEnabled && parsed.requireOnWake;
      }
    } catch(e) {}
    return false;
  });
  ...
  const [energyLevel, setEnergyLevel] = useState<EnergyLevel>(() => {
    try {
      const saved = localStorage.getItem('timenest_energy');
      if (saved) return saved as EnergyLevel;
    } catch(e) {}
    return 'Média';
  });

  // Save profile, security, and achievements whenever they change
  ...
  useEffect(() => {
    localStorage.setItem('timenest_energy', energyLevel);
  }, [energyLevel]);
