...
export const FocusProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [stats, setStats] = useState<FocusStats>({ focusMinutesToday: 0, completedCycles: 0 });
    ...
    try {
      ...
      if (savedStats) {
        ...
        if (parsed.date === new Date().toISOString().split('T')[0]) {
          setStats({ focusMinutesToday: parsed.focusMinutesToday, completedCycles: parsed.completedCycles });
        }
      }
  ...
  return (
    <FocusContext.Provider value={{
      activeTask, isActive, isPaused, focusType, setFocusType,
