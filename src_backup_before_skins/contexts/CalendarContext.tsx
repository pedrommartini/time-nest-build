...
export const CalendarProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Load events based on environment
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (isTestEnvironment) {
        ...
        setEvents(mockTestEvents);
        localStorage.setItem(key, JSON.stringify(mockTestEvents));
      } else {
        // Initial setup for non-test mode (if empty)
        setEvents([]);
      }
    }
  }, [isTestEnvironment]);
  ...
  useEffect(() => {
    ...
    if (savedSync) setGoogleSync(JSON.parse(savedSync));
  ...
  return (
    <CalendarContext.Provider value={{
      events, freeIntervals, addEvent, updateEventTimes, deleteEvent,
